...
class Program
{
    static async Task Main()
    {
        ...
        string windAdvice = Wind.WindSpeed((int)wind);
        ...
        string timeText = $"{greeting}Klokken er {now:HH:mm}.Temperaturen er {temp}°C.Nedbør?:{(rain ? "Ja." : "Nei")} ({precip} mm. {jakkeSnakk}{windAdvice}";


        // Speak the time using macOS's built-in 'say' command
        Process.Start("say", "-r 130" + timeText);
